...
def validate_public_url(raw: str) -> str:
    ...
    try:
        ...
        if not addresses:
            raise ValueError("url host did not resolve") from None
...
def normalize_datetime(date_value: str, time_value: str = "") -> str:
    ...
    if not parsed:
        return squash(f"{date_value} {time_value}")
...
def headline(text: str) -> str:
    ...
    for line in lines[:8]:
        if len(line) < 8:
            continue
        if re.match(
            r"^(description of the service|date of issuance|closing date and time|rfb number|telephone number|contact persons|table of contents|page\s+\|)",
            line,
            flags=re.I,
        ):
            break
...
def mine(text: str) -> dict:
    facts = {}
    pats = {
        "closing_details": r"(closing[^.\n]{0,160})",
        "briefing_details": r"(briefing[^.\n]{0,160})",
        "submission_details": r"(submission[^.\n]{0,160})",
        "contact_details": r"((?:email|tel|contact)[^.\n]{0,160})",
        "cidb_hints": r"((?:cidb|grading)[^.\n]{0,160})",
    }
    for k, p in pats.items():
        m = re.search(p, text, flags=re.I)
        if m:
            facts[k] = squash(m.group(1))[:240]

    title = headline(text)
    if title:
        facts["document_title"] = title[:240]
    ...
    m = capture(
        text,
        [
            r"(?:date of issuance|date published|date advertised)\s*[:\-]?\s*([0-9]{1,2}[\/-][0-9]{1,2}[\/-][0-9]{2,4}|[0-9]{1,2}\s+[A-Za-z]+\s+[0-9]{4})"
        ],
    )
    if m:
        facts["issued_date"] = normalize_date(m.group(1))

    m = capture(
        text,
        [
            r"(?:validity(?: period)?|proposal validity)[^0-9]{0,20}(\d+)\s*days",
            r"remain valid[^0-9]{0,20}(\d+)\s*days",
        ],
    )
    if m:
        facts["validity_days"] = m.group(1)
    ...
    m = capture(
        text,
        [
            r"minimum\s+functionality(?:\s+score)?[^0-9%]{0,30}(\d+(?:\.\d+)?)\s*%",
            r"minimum threshold of\s+(\d+(?:\.\d+)?)\s*points",
        ],
    )
    if m:
        facts["minimum_functionality_score"] = m.group(1)

    m = capture(text, [r"\bCIDB\b[^.\n]{0,80}\b(\d+\s*[A-Z]{1,3})\b"])
    if m:
        facts["cidb_grade"] = squash(m.group(1)).replace(" ", "")

    emails = EMAIL_RE.findall(text)
    if emails:
        facts["contact_email"] = emails[0]
    phones = [squash(m.group(0)) for m in PHONE_RE.finditer(text)]
    if phones:
        facts["contact_phone"] = phones[0]
    ...
    if m:
        facts["contact_name"] = squash(m.group(1))
    ...
    m = capture(
        text,
        [
            r"(?:Time of completion for this contract is|period of)\s+(\d+)\s*(weeks?|months?|years?)"
        ],
    )
    if m:
        facts["contract_duration"] = f"{m.group(1)} {m.group(2)}"
